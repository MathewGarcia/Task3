import ArrayFilter from './componets/ArrayFilter'
import ArrayReducer from './componets/ArrayReducer'
import ArraySort from './componets/ArraySort'
import './App.css'
import React, {useState} from 'react'
import ArrayMap from './componets/ArrayMap'
  ...
  return (
    ...
    <nav className="flex justify-center gap-5">
      ...
      <button onClick={arrayReduceHandler} className="relative inline-block text-lg group">
        ...
        <span className="absolute inset-0 w-full h-full px-5 py-3 rounded-lg bg-gray-50"></span>
        <span className="absolute left-0 w-48 h-48 -ml-2 transition-all duration-300 origin-top-right -rotate-90 -translate-x-full translate-y-12 bg-gray-900 group-hover:-rotate-180 ease"></span>
        <span className="relative">Array Reducer</span>
        </span>
        <span className="absolute bottom-0 right-0 w-full h-12 -mb-1 -mr-1 transition-all duration-200 ease-linear bg-gray-900 rounded-lg group-hover:mb-0 group-hover:mr-0" data-rounded="rounded-lg"></span>
      </button>
    ...
    <section className='py-5'>
      ...
      {componentToShow === 'reduce' && <ArrayReducer />}
